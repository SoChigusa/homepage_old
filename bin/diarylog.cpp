...
void errorMessage() {
  ...
  std::cout << "./diarylog update: update files using existing log"
            << std::endl;
}
...
void update_log(const std::string &comment) {
  ...
  system(
      "cp ../diary/index.html ../../sochigusa.bitbucket.org/diary/index.html");
  system("cp ../diary/page-* ../../sochigusa.bitbucket.org/diary/");
  system(("git add ../diary/page-* && git commit -a -m \"auto commit by "
          "diarylog : " +
          comment + "\" && " + "git push origin main")
             .c_str());
  system(("cd ../../sochigusa.bitbucket.org/ && git add diary/page-* && git "
          "commit -a -m \"auto commit by diarylog : " +
...
int main(int argc, char **argv) {
  ...
  if (arg_opt == "add") {
    ...
    if (!ifcont.is_open()) {
      std::cout << "File ../diary/source/" << arg_name << ".html isn't found"
                << std::endl;
      return -1;
    }
